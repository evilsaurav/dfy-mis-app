...
if start_main_dash != -1 and end_main_dash != -1:
    ...
    # Also we need to strip `appState === 'completed' ? ( ... ) : (` because the old structure had it!
    # Wait, the structure was:
    # `) : appState === 'completed' ? ( ... ) : ( /* Main Dashboard */`
    start_completed = text.find('                ) : appState === \'completed\' ? (')
    if start_completed != -1:
        text = text[:start_completed] + new_dash + text[end_main_dash:]
    else:
