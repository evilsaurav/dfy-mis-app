...
# Now, we manually rewrite the main UI rendering part.
# The rendering part starts at: `        ) : (`  after Login screen
import re
...
if start_main_dash != -1 and end_main_dash != -1:
    # We will slice out the entire main dash and rewrite it cleanly.
    # To rewrite it safely, we'll extract the pieces we need: the Accordions and the Doctors section.
    
    # Extract Group 1 Accordion
    ...
    # Extract Remark
    rem_start = text.find('                  <div className="bg-white rounded-2xl shadow')
    rem_end = text.find('                  </div>', rem_start) + 24
    # Actually wait, the remark is just a div.
    # Let's search for "Overall Remark"
    ...
    # Extract Doctor visits
    doc_start = text.rfind('<div className="bg-white rounded-2xl', 0, text.find('Doctor / Store Visits'))
    doc_end = text.find('</div>\n                </div>\n              )}', doc_start) 
    # Actually doc_end is when the inner divs close.
