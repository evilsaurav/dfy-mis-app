    ...
    <div className="w-full max-w-lg mx-auto animate-fade-in pb-10">
      ...
      {stats && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4 px-1">
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider flex items-center gap-1.5 shrink-0">
          ...
          <div className="flex flex-wrap items-center gap-2 text-[9px] font-bold text-slate-400">
            ...
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-amber-500"></span> Medical Leave</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-sky-500"></span> Casual / Annual</span>
            <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded-full bg-indigo-500"></span> Official Duty</span>
              ...
              {Array.from({ length: daysInMonth }, (_, i) => {
                ...
                const cellTitle = (() => {
                  if (dayData && dayData.submitted) {
                    return `${dateKey}: ${count} report(s), ${dayData.total_ids} IDs (Click to view)`;
                  }
                  if (dayData && dayData.is_leave) {
                    const r = dayData.reason_type || dayData.leave_info?.reason_type || (dayData.status === 'absent' ? 'Absent' : 'Leave');
                    return `${dateKey}: ${r} (Click to view)`;
                  }
                  return `${dateKey}: No report`;
                })();
      ...
      {selectedDate && (() => {
        ...
        return (
          <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-100 mb-6 animate-fade-in">
            <div className="flex justify-between items-center pb-3 border-b border-slate-100 mb-3">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-semibold">
                  {selectedDayData && selectedDayData.submitted ? `${selectedDayData.total_ids} Total IDs Recorded` : selectedDayData && selectedDayData.is_leave ? 'Leave marked for this date' : 'No report submitted on this date'}
                </p>
              </div>
            ...
            {selectedDayData && selectedDayData.is_leave && (
              <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200/80 rounded-2xl p-4 my-3 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-black uppercase text-amber-900 flex items-center gap-1.5">
                    <span>🏖️</span> Approved Leave: {selectedDayData.reason_type || selectedDayData.leave_info?.reason_type || (selectedDayData.status === 'absent' || selectedDayData.leave_info?.status === 'absent' ? 'Absent' : 'Leave')}
                  </span>
                  <span className="text-[10px] font-bold text-amber-700 bg-amber-100 px-2 py-0.5 rounded-full border border-amber-300">
                    Marked by {selectedDayData.marked_by || selectedDayData.leave_info?.marked_by || 'Admin'}
                  </span>
                </div>
                {(selectedDayData.remark || selectedDayData.leave_info?.remark) && (
            ...
            {selectedDayData && selectedDayData.submitted ? (
              ...
              <p className="text-center py-6 text-xs text-amber-800 font-medium">Is date ko Admin dwaara leave mark ki gayi hai.</p>
            ) : (
